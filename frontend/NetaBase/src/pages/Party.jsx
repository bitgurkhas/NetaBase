...
export default function PoliticalPartiesSection() {
  ...
  const fetchParties = async () => {
    try {
      ...
      const response = await axios.get(`${baseUrl}/api/parties/`);
      setParties(response.data.results || []);
    } catch (err) {
      setError(err.response?.data?.detail || 'Failed to fetch parties');
  ...
  const fetchPartyPoliticians = async (slug) => {
    try {
      setLoadingPoliticians(true);

      const response = await axios.get(`${baseUrl}/api/parties/${slug}/politicians/`);
